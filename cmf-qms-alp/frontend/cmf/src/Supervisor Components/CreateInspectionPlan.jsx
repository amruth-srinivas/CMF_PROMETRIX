...
import { SearchOutlined, BuildOutlined, EyeOutlined } from '@ant-design/icons';
...
const CreateInspectionPlan = () => {
  ...
  const fetchActiveProjects = async () => {
    setLoading(true);
    try {
      // Fetch status of all orders/parts to find active ones
      // Using the specific endpoint requested by the user
      const host = window.location.hostname;
      // The user specified http://172.18.100.31:8986/api/v1/scheduling/orders-parts-status
      // We'll use the QUALITY_API_BASE_URL which is likely on port 8986
      const response = await axios.get(`${QUALITY_API_BASE_URL}/scheduling/orders-parts-status`);
      const allEntries = response.data.orders || [];

      // Filter for entries where status is 'active'
      const activeEntries = allEntries.filter(entry => entry.status === 'active');

      // Group by sale_order_id to get a unique list of active projects
      const projectsMap = {};
      activeEntries.forEach(entry => {
        if (!projectsMap[entry.sale_order_id]) {
          projectsMap[entry.sale_order_id] = {
            ...
            // Initialize other fields with placeholders
            qty: '-',
            customer: '-',
            projectCoordinator: '-',
            orderDate: '-',
      ...
      // Enrich with more details from /orders/ endpoint
      try {
  ...
  const handleCreatePlan = (record) => {
    navigate(`/supervisor/quality-management?productId=${record.productId}&orderId=${record.saleOrderId}`);
  };
      ...
      <Card
        title={
          <Space>
            ...
            <Title level={4} style={{ margin: 0 }}>Create Inspection Plan</Title>
          </Space>
        }
        extra={
          <Input
